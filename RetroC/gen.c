...
static int genIF(struct ASTnode *n){
    ...
    //Generate two labels: one for the false compound statement and one for
    //the end of the overall IF statement. When there is no ELSE clause, Lfalse is the ending label
    Lfalse = label();
    if(n->right)
        Lend = label();

    //Generate the condition code followed by a zero jump to the false label; cheats by sending Lfalse label as a register
...
static int genWHILE(struct ASTnode *n){
    ...
    //Generate the condition code followed by a jump to the end label; cheats by sending the Lfalse label as a register
